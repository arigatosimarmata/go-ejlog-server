...
func NcrProcessEjlogElastic(ejcontent, ip_address, kanwil string) error {
	keywordMapNcr := models.KeywordEjol
	date := time.Now()
	...
	for _, ejlog := range ejol_map {
		...
		for key, value := range (*keywordMapNcr)["NCR_KEYWORD"] {

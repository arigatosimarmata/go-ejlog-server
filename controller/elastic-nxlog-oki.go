...
func OkiProcessEjlogElastic(ejcontent, ip_address, kanwil string) error {
	keywordMapOki := models.KeywordEjol
	date := time.Now()
	...
	for _, ejlog := range ejol_map {
		...
		for key, value := range (*keywordMapOki)["OKI_KEYWORD"] {

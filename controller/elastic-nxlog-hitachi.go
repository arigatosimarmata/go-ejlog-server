...
func HitachiParseProcess() error {
...
func HitachiProcessEjlogElastic(ejcontent, ip_address, kanwil string) error {
	keywordMapHitachi := models.KeywordEjol
	date := time.Now()
	...
	for _, ejlog := range ejol_map {
		...
		for key, value := range (*keywordMapHitachi)["HITACHI_KEYWORD"] {
